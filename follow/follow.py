...
class FollowerNode(Node):
    def __init__(self):
        ...
        # ── 可調參數 ──────────────────────────────────────────
        self.leader_name   = 'wamv'   # Leader  boat name
        self.follower_name = 'wamv2'  # Follower boat name
        self.target_distance = 8.0    # 期望保持的距離 (公尺)
        self.max_thrust   = 1000.0    # WAM-V 推力上限 (N)
        self.k_p_linear   = 80.0     # 線性 P 增益
        self.k_p_angular  = 300.0    # 角度 P 增益
        # ──────────────────────────────────────────────────────

        # 訂閱 Leader GPS
        self.create_subscription(
            NavSatFix,
            f'/{self.leader_name}/sensors/gps/gps/fix',
            self.leader_gps_callback,
            qos_profile_sensor_data)

        # 訂閱 Follower GPS + IMU
        self.create_subscription(
            NavSatFix,
            f'/{self.follower_name}/sensors/gps/gps/fix',
            self.follower_gps_callback,
            qos_profile_sensor_data)
        self.create_subscription(
            Imu,
            f'/{self.follower_name}/sensors/imu/imu/data',
            self.follower_imu_callback,
            qos_profile_sensor_data)

        # 發布推力與轉向指令給 Follower
        self.left_thrust_pub  = self.create_publisher(Float64, f'/{self.follower_name}/thrusters/left/thrust',  10)
        self.right_thrust_pub = self.create_publisher(Float64, f'/{self.follower_name}/thrusters/right/thrust', 10)
        self.left_pos_pub     = self.create_publisher(Float64, f'/{self.follower_name}/thrusters/left/pos',     10)
        self.right_pos_pub    = self.create_publisher(Float64, f'/{self.follower_name}/thrusters/right/pos',    10)
        ...
        self.get_logger().info(
            f"跟隨者節點已啟動  leader={self.leader_name}  follower={self.follower_name}")
    ...
    def leader_gps_callback(self, msg):
        ...
        if self.origin_lat is None:
            ...
            self.get_logger().info(
                f"設定原點: lat={self.origin_lat:.6f}, lon={self.origin_lon:.6f}")
    ...
    def control_loop(self):
        ...
        if distance > self.target_distance:
            forward_thrust = self.k_p_linear * (distance - self.target_distance)

        turn_thrust = self.k_p_angular * heading_error
        ...
        self.get_logger().info(
            ...
            f"  L={left_cmd:.0f}N  R={right_cmd:.0f}N")
